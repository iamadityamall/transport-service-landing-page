...
const Sidebar = ({ setNavButton, navButton }) => {
    const {changemode, darkmode, } = useGlobalContext();
  return (
    <aside
      className={`fixed text-black top-0 right-0 h-screen space-y-28 w-screen p-4 transition all delay-300 ease-in-out flex flex-col items-end text-right capitalize lg:hidden bg-white ${
    ...
    >
      ...
      <div className="flex flex-col w-full items-end space-y-3">
        <button
          className={`text-5xl text-colorOne   ${darkmode && "text-black"}`}
          onClick={() => changemode()}
        >
          <MdDarkMode />
        </button>
